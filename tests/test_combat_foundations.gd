...
func test_corpse_darkens_toward_black_as_it_decays() -> void:
	var victim := GameManager.spawn_unit(TANK_STATS, GameManager.get_player(GameManager.BLUE_TEAM_ID), Vector3.ZERO)
	var brightness_at_spawn := _channel_sum(victim._body_material.get_shader_parameter("team_color"))

	victim.take_damage(DamageInstance.new(victim.stat_block.max_health() + 100.0))
	assert_eq(_channel_sum(victim._body_material.get_shader_parameter("team_color")), brightness_at_spawn, "should still be full team color the instant it dies")
	...
	for i in range(10): # short waits, well clear of the 3s decay actually finishing (and freeing the corpse out from under this test)
		await wait_physics_frames(1)
	var first_check := _channel_sum(victim._body_material.get_shader_parameter("team_color"))
	...
	for i in range(10):
		await wait_physics_frames(1)
	assert_lt(_channel_sum(victim._body_material.get_shader_parameter("team_color")), first_check, "should keep getting darker as decay progresses")
